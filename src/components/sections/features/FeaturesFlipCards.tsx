...
const FeatureFlipCard = ({ item }: { item: FeatureItem }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col gap-3 xl:gap-4 2xl:gap-5 p-3 xl:p-4 2xl:p-5 card rounded backface-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-between gap-3 xl:gap-4 2xl:gap-5 p-3 xl:p-4 2xl:p-5 card rounded backface-hidden transform-[rotateY(180deg)]">
          <div className="flex items-start justify-between gap-5">
            <h3 className="text-2xl font-medium leading-tight">{item.title}</h3>
            <div className="flex items-center justify-center shrink-0 size-8 primary-button rounded">
          ...
          <div className="flex flex-col gap-3">
            {item.descriptions.map((desc, index) => (
              <p key={index} className="text-lg leading-tight">{desc}</p>
            ))}
          </div>
        </div>
      </div>
